...
const authToken = Cookies.get('authToken');
    ...
    const fetchNotifications = async (authToken) => {
      setLoading(true);
      try {
        const response = await fetch('https://fis.metaforeignoption.com/api/notification', {
          headers: {
...
const adminNotifications = data.filter(notification => notification.receiver && notification.receiver.role === 'admin');
  ...
  // Slice the notifications array to display only the notifications for the current page
  ...
  return (
   ...
   {notificationsForPage.map(notification => (
        <Box key={notification._id} className="left-box">
          ...
          <Box className='Box1' style={{ display: 'flex', flexDirection: 'column', position: 'relative' }}>
            <Box style={{ display: 'flex',  alignItems:'center' }}>
              <Box>
                <p style={{ marginLeft: '5rem' }}>@{notification.sender?.username}</p>
              </Box>
         <Typography style={{ fontSize: '10px', color: '#4A0808', justifyContent: 'center', marginLeft:'1rem' }} className='text'>
...
<p style={{ color: 'grey' }}>
 ...
 {/* Pagination */}
      <Box display="flex" justifyContent="center" mt={2}>
        <Pagination count={Math.ceil(notifications.length / notificationsPerPage)} page={page} onChange={handlePageChange} />
      </Box>

      {/* Comments dialog/modal */}
      {/* Your existing code for the dialog/modal here */}

      {/* Loading indicator */}
      {loading && <CircularProgress style={{display: 'flex', justifyContent: 'center', alignItems:'center', marginLeft:'35vw', 
      marginTop: '20vh'}} />}
